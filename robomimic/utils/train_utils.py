...
def run_rollout(
    ...
    total_reward = 0.
    success = { k: False for k in env.is_success() } # success metrics  {"task": False}
    ...
    try:
        for step_i in range(horizon):
            ...
            cur_success_metrics = env.is_success()  # {'task': True}
            for k in success:
...
def rollout_with_stats(
    ...
    if video_dir is not None:
        ...
        video_paths = {k: os.path.join(video_dir, "{}{}".format(k, video_str)) for k in envs}    # {'Lift': /Lift_epoch_1.mp4}
        video_writers = {k: imageio.get_writer(video_paths[k], fps=20) for k in envs}    # {'Lift': }
    ...
    for env_name, env in envs.items():
        ...
        for ep_i in iterator:
            ...
            )   # {"Return": total_reward, "Horizon": 100, "Success_Rate": 1/0}
        ...
        rollout_logs = dict((k, [rollout_logs[i][k] for i in range(len(rollout_logs))]) for k in rollout_logs[0])   # {"Return": [10,11,...], "Horizon": [100,100,...], "Success_Rate": [1,0,...]}
        rollout_logs_mean = dict((k, np.mean(v)) for k, v in rollout_logs.items())
        rollout_logs_mean["Time_Episode"] = np.sum(rollout_logs["time"]) / 60. # total time taken for rollouts in minutes
        all_rollout_logs[env_name] = rollout_logs_mean  # {'Lift': {"Return": 1.0, "Horizon": 100.0, "Success_Rate": 1.0}}
    ...
    for env_name in all_rollout_logs:
        ...
        if rollout_logs["Success_Rate"] > best_success_rate[env_name]:
            ...
            if save_on_best_rollout_success_rate:
                ...
                epoch_ckpt_name += "_{}_success_{}".format(env_name, best_success_rate[env_name])   # 'model_epoch_1_Lift_success_1.0'
                should_save_ckpt = True
                ckpt_reason = "success"
...
def run_epoch(model, data_loader, epoch, validate=False, num_steps=None, obs_normalization_stats=None):
    ...
    for _ in LogUtils.custom_tqdm(range(num_steps)):
        ...
        try:
            t = time.time()
            batch = next(data_loader_iter)  # {'actions': (100, 1, 7), 'rewards': (100, 1), 'dones': (100, 1), 'obs': {'object': (100, 1, 10), 'robot0_eef_pos': (100, 1, 3), ...}}
        ...
        t = time.time()
        input_batch = model.process_batch_for_training(batch)   # {'actions': (100, 7), 'obs': {{'object': (100, 10), 'robot0_eef_pos': (100, 3), ...}}}
    ...
    step_log_all = dict((k, float(np.mean(v))) for k, v in step_log_dict.items())   # {"Cosine_Loss","L1_Loss","L2_Loss","Loss",","Optimizer/policy0_lr"}

    # add in timing stats
    for k in timing_stats:
        # sum across all training steps, and convert from seconds to minutes
        step_log_all["Time_{}".format(k)] = np.sum(timing_stats[k]) / 60.   # {"Time_Data_Loading", "Time_Epoch", "Time_Log_Info", "Time_Process_Batch"}
