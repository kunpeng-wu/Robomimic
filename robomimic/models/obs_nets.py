...
def obs_encoder_factory(
    ...
    for k, obs_shape in obs_shapes.items():     # 'object', [10]
        obs_modality = ObsUtils.OBS_KEYS_TO_MODALITIES[k]   # 'low_dim'
        enc_kwargs = deepcopy(ObsUtils.DEFAULT_ENCODER_KWARGS[obs_modality]) if encoder_kwargs is None else \
            deepcopy(encoder_kwargs[obs_modality])  # {"core_class": null, "core_kwargs": {}, "obs_randomizer_class": null, "obs_randomizer_kwargs": {}}
...
class ObservationEncoder(Module):
    ...
    def register_obs_key(
        ...
        self.obs_shapes[name] = shape   # {"object": [10], "robot0_eef_pos": [3], "robot0_eef_quat": [4], "robot0_gripper_qpos": [2]}
        self.obs_nets_classes[name] = net_class     # {"object": None, "robot0_eef_pos": None, "robot0_eef_quat": None, "robot0_gripper_qpos": None}
        self.obs_nets_kwargs[name] = net_kwargs     # {"object": {"input_shape": [10]}, "robot0_eef_pos": {"input_shape": [3]}, "robot0_eef_quat": {"input_shape": [4]}, "robot0_gripper_qpos": {"input_shape": [2]}}
        self.obs_nets[name] = net   # {"object": None, "robot0_eef_pos": None, "robot0_eef_quat": None, "robot0_gripper_qpos": None}
        self.obs_randomizers[name] = randomizer     # {"object": None, "robot0_eef_pos": None, "robot0_eef_quat": None, "robot0_gripper_qpos": None}
        self.obs_share_mods[name] = share_net_from  # {"object": None, "robot0_eef_pos": None, "robot0_eef_quat": None, "robot0_gripper_qpos": None}
    ...
    def forward(self, obs_dict):
        ...
        for k in self.obs_shapes:
            ...
            feats.append(x)     # [Tensor(100, 10), Tensor(100, 3), ...]
...
class ObservationDecoder(Module):
    ...
    def __init__(
        ...
        for k in decode_shapes:
            self.obs_shapes[k] = decode_shapes[k]   # {'action': (7,)}
...
class MIMO_MLP(Module):
    ...
    def __init__(
        ...
        self.input_obs_group_shapes = input_obs_group_shapes    # {'obs': {'object': [10], 'robot0_eef_pos': [3]}}
        self.output_shapes = output_shapes  # {'action': (7,)}
        ...
        mlp_input_dim = self.nets["encoder"].output_shape()[0]  # 10 + 3 + 4 + 2 = 19
    ...
    def forward(self, **inputs):
        ...
        """
        enc_outputs = self.nets["encoder"](**inputs)    # (100, 19)
        mlp_out = self.nets["mlp"](enc_outputs)     # (100, 1024)
        return self.nets["decoder"](mlp_out)    # (100, 7)
